...
public class NetWorkTools {
	...
	public static int getCurrentNetWorkType(Context mContext){
		if (mContext == null) {
			TCPLog.v("getCurrentNetWorkType mContext is null!!!");
			return NETWORK_ON;
		}
...
//		}
		if (activeNetInfo != null && activeNetInfo.isConnected()) {
			if ("WIFI".equalsIgnoreCase(activeNetInfo.getTypeName())) {
				...
				netSubtype = activeNetInfo.getSubtype();
				TCPLog.v("getCurrentNetWorkType getTypeName():" + activeNetInfo.getTypeName() + " getSubtype():" + + activeNetInfo.getSubtype());
		...
		TCPLog.v("currentNetWorkType:" + currentNetWorkType);
		return currentNetWorkType;
	}
	
	private static NetworkInfo getNetworkInfo(Context mContext){
		if (mContext == null) {
			TCPLog.v("getNetworkInfo mContext is null!!!");
			return null;
		}
	...
	public static boolean isNetWorkConnect(Context mContext){
		if (mContext == null) {
			TCPLog.v("isNetWorkConnect mContext is null!!!");
			return false;
		}
		NetworkInfo activeNetInfo =  getNetworkInfo(mContext);
	           ...
	           for (NetworkInterface intf : interfaces) {
	               ...
	               for (InetAddress addr : addrs) {
	                   if (!addr.isLoopbackAddress()) {
	                       ...
	                       TCPLog.v("isIPv4:" + isIPv4 + " sAddr:" + sAddr);
	                       if (useIPv4) {
	                           if (isIPv4) {
	                               return sAddr;
	                           }
	...
	public static boolean isConnectWifi(Context mcontext) {
		if (mcontext == null) {
			TCPLog.v("isConnectWifi mcontext is null!!!");
			return false;
		}
