...
public class AudioDeviceParam {
    ...
    public static AudioDeviceParam getInstance() {
        if ( mSingle == null ) {
            ...
            CustomLog.i(TAG, "getInstance: Create!!");
        } else {
            CustomLog.i(TAG, "getInstance: Single already exist!!");
        }
    ...
    public synchronized void setNoRecordEnable(boolean enable) {
    	bNoRecord = enable;
    }
    
    public synchronized void setNoTrackEnable(boolean enable) {
    	bNoTrack = enable;
    }
    /**
