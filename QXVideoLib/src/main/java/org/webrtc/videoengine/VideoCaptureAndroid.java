...
public class VideoCaptureAndroid implements PreviewCallback, Callback {
  ...
  private int mCameraOrientation = 270; // default front camera
  private static int rotation_local_preview=0;
  ...
  public static void setLocalPreviewRotation(int rotation_camera_)
  {
	  rotation_local_preview = rotation_camera_;
  }
  ...
  private synchronized boolean startCapture(
    ...
    CustomLog.i("startCapture: " + width + "x" + height + "@" +
        min_mfps + ":" + max_mfps);
...
//      height = 480;
  ...
  private void stopCaptureOnCameraThread(
    ...
    CustomLog.i(TAG, "Failed to stop camera, error:" + error.getMessage());
    exchange(result, false);
    Looper.myLooper().quit();
    return;
  }
...
//  Log.i(TAG, "onPreviewFrame rotation = " + rotation + " info.orientation = " + info.orientation);
  ...
  private void setPreviewRotationOnCameraThread(
    ...
    camera.setDisplayOrientation( (resultRotation+rotation_local_preview) % 360);
    exchange(result, null);
  }
